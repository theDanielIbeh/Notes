...
interface AttachmentDao {
    ...
    @Query(
        "DELETE FROM ${AttachmentTableConstants.ATTACHMENT_TABLE} " +
            "WHERE ${AttachmentTableConstants.ID} = :id",
    )
    suspend fun deleteAttachmentById(id: Int)

    @Query(
        "DELETE FROM ${AttachmentTableConstants.ATTACHMENT_TABLE} " +
            "WHERE ${AttachmentTableConstants.NOTE_ID} = :noteId",
    )
    suspend fun deleteAttachmentByNoteId(noteId: Int)

    @Query(
        "SELECT * FROM ${AttachmentTableConstants.ATTACHMENT_TABLE} " +
            "WHERE ${AttachmentTableConstants.ID} = :noteId " +
            "ORDER BY ${AttachmentTableConstants.TIME_STAMP} DESC",
    )
    ...
    @Query(
        "SELECT * FROM ${AttachmentTableConstants.ATTACHMENT_TABLE} " +
            "WHERE ${AttachmentTableConstants.NOTE_ID} = :noteId " +
            "AND ${AttachmentTableConstants.URI} = :uri",
    )
    ...
    @Query(
        "SELECT * FROM ${AttachmentTableConstants.ATTACHMENT_TABLE} " +
            "WHERE ${AttachmentTableConstants.TIME_STAMP} = :timeStamp",
    )
